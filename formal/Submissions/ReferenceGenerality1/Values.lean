import Submissions.ReferenceGenerality1.Tree
import Submissions.ReferenceGenerality1.Keygen
import Submissions.ReferenceGenerality1.SignIdx
...
hash node is read back from the tweak word in its 128 high bits (`tagNat_pointOf`, `tagging`), and a
...
/-- A tweak word node holds its constant. -/
theorem val_det_const (ξ : Rec) (n : Name) (hn : n.isTw = true) :
    val ξ n = detVal n (graph.evalRec ξ) := by
  unfold val
  rw [evalRec_apply_fin]
  cases n <;> simp only [Name.isTw, Bool.false_eq_true] at hn <;>
    simp only [kindOf, detKind, NodeKind.value] <;> exact cast_cast_eq _ _ _

theorem val_tc (ξ : Rec) (k : Fin 63) (t : Fin 14) : val ξ (tc k t) = tw (ch k t) :=
  val_det_const ξ (tc k t) rfl

theorem val_tg (ξ : Rec) (j : Fin 21) : val ξ (tg j) = tw (gh j) :=
  val_det_const ξ (tg j) rfl

theorem val_te (ξ : Rec) (l : Fin 7) : val ξ (te l) = tw (eh l) :=
  val_det_const ξ (te l) rfl

theorem val_tr (ξ : Rec) : val ξ tr = tw rh :=
  val_det_const ξ tr rfl

theorem trunc_evalRec_tc (ξ : Rec) (k : Fin 63) (t : Fin 14) :
    trunc (graph.evalRec ξ (tc k t).fin) = tw (ch k t) := by
  rw [trunc_evalRec]
  exact (congrArg trunc (val_tc ξ k t)).trans (trunc_eq_self _)

theorem trunc_evalRec_tg (ξ : Rec) (j : Fin 21) :
    trunc (graph.evalRec ξ (tg j).fin) = tw (gh j) := by
  rw [trunc_evalRec]
  exact (congrArg trunc (val_tg ξ j)).trans (trunc_eq_self _)

theorem trunc_evalRec_te (ξ : Rec) (l : Fin 7) :
    trunc (graph.evalRec ξ (te l).fin) = tw (eh l) := by
  rw [trunc_evalRec]
  exact (congrArg trunc (val_te ξ l)).trans (trunc_eq_self _)

theorem trunc_evalRec_tr (ξ : Rec) : trunc (graph.evalRec ξ tr.fin) = tw rh := by
  rw [trunc_evalRec]
  exact (congrArg trunc (val_tr ξ)).trans (trunc_eq_self _)
...
theorem val_ci (ξ : Rec) (k : Fin 63) (t : Fin 14) :
  ...
  unfold val
  rw [evalRec_apply_fin]
  simp only [kindOf, detKind, NodeKind.value]
  refine (cast_cast_eq _ _ _).trans ?_
  show (trunc (graph.evalRec ξ (tc k t).fin) ++ trunc (graph.evalRec ξ (prev k t).fin) :
    BitVec (128 + 128)) = _
  rw [trunc_evalRec_tc, trunc_evalRec]
  rfl
...
theorem val_gc (ξ : Rec) (j : Fin 21) :
  ...
  simp only [kindOf, detKind, NodeKind.value]
  refine (cast_cast_eq _ _ _).trans ?_
  show (trunc (graph.evalRec ξ (tg j).fin) ++ cat3 (trunc (graph.evalRec ξ (cv (chainOf j 0) 13).fin))
    (trunc (graph.evalRec ξ (cv (chainOf j 1) 13).fin))
    (trunc (graph.evalRec ξ (cv (chainOf j 2) 13).fin)) : BitVec (128 + 384)) = _
  rw [trunc_val_cv, trunc_val_cv, trunc_val_cv, trunc_evalRec_tg]
...
theorem val_ec (ξ : Rec) (l : Fin 7) :
  ...
  simp only [kindOf, detKind, NodeKind.value]
  refine (cast_cast_eq _ _ _).trans ?_
  show (trunc (graph.evalRec ξ (te l).fin) ++ cat3 (trunc (graph.evalRec ξ (gv (groupOf l 0)).fin))
    (trunc (graph.evalRec ξ (gv (groupOf l 1)).fin))
    (trunc (graph.evalRec ξ (gv (groupOf l 2)).fin)) : BitVec (128 + 384)) = _
  rw [trunc_val_gv, trunc_val_gv, trunc_val_gv, trunc_evalRec_te]
...
theorem val_rc (ξ : Rec) : val ξ rc = tw rh ++ cat7 fun l => trunc (ξ.2 (eh l).fin) := by
  ...
  simp only [kindOf, detKind, NodeKind.value]
  refine (cast_cast_eq _ _ _).trans ?_
  show (trunc (graph.evalRec ξ tr.fin) ++ cat7 (fun l => trunc (graph.evalRec ξ (ev l).fin)) :
    BitVec (128 + 896)) = _
  rw [trunc_evalRec_tr]
...
/-- The input of a hash node has length 256 (chains), 512 (groups, subtrees) or 1024 (root). -/
theorem len_hashParent_cases {h p : Name} (hp : hashParent h = some p) :
    p.len = 256 ∨ p.len = 512 ∨ p.len = 1024 := by
  cases h <;> simp only [hashParent, Option.some.injEq, reduceCtorEq] at hp <;> subst hp <;>
    simp [Name.len]
...
/-- A keygen point is not an index query: its length is 256, 512 or 1024, never 384. -/
...
theorem graph_point_fin (ξ : Rec) (h : Name) :
  ...
  unfold Graph.point
  rw [graph_kind_fin]
  cases h <;> simp only [kindOf, detKind, hashParent, Option.map_some, Option.map_none, pointOf]
...
/-- The hash node a query belongs to: the node whose index is written in its 128 high bits. -/
...
theorem graph_kind_eq_hash {h : Name} {q : Fin N} {hq : q < h.fin} {hl : lenF h.fin = 256}
  ...
  cases h <;> simp only [kindOf, detKind, reduceCtorEq] at hk
...
/-- The tweak word of a hash node is a parent of its input. -/
theorem twOf_mem_parents {h p : Name} (hp : hashParent h = some p) : twOf h ∈ Name.parents p := by
  cases h <;> simp only [hashParent, Option.some.injEq, reduceCtorEq] at hp <;> subst hp <;>
    simp [Name.parents, twOf]

/-- The tweak word of a hash node is a tweak word. -/
theorem isTw_twOf {h p : Name} (hp : hashParent h = some p) : (twOf h).isTw = true := by
  cases h <;> simp only [hashParent, Option.some.injEq, reduceCtorEq] at hp <;> rfl

/-- The tweak word node of a hash node is a constant: its value is the tweak. -/
theorem detVal_twOf {h p : Name} (hp : hashParent h = some p) (x : Asg) :
    trunc (detVal (twOf h) x) = tw h := by
  cases h <;> simp only [hashParent, Option.some.injEq, reduceCtorEq] at hp <;>
    exact trunc_eq_self _

/-- An assignment in which the tweak word of `h` has its constant value. -/
theorem trunc_twOf_of_constAt {h p : Name} (hp : hashParent h = some p) (x : Asg)
    (hx : graph.ConstAt x (twOf h).fin) : trunc (x (twOf h).fin) = tw h := by
  have hk : graph.kind (twOf h).fin = detKind (twOf h).fin (twOf h) (ofFin_fin _) := by
    rw [graph_kind_fin]
    cases h <;> simp only [hashParent, Option.some.injEq, reduceCtorEq] at hp <;> rfl
  have e := hx _ _ _ _ hk (by
    cases h <;> simp only [hashParent, Option.some.injEq, reduceCtorEq] at hp <;>
      exact Finset.map_empty _)
  rw [e, trunc_cast, detVal_twOf hp]

/-- The input of a hash node carries its tweak, when the tweak word has its value. -/
theorem tagNat_detVal_of_hashParent {h p : Name} (hp : hashParent h = some p) (x : Asg)
    (hx : trunc (x (twOf h).fin) = tw h) :
    tagNat ⟨p.len, detVal p x⟩ = h.idx :=
  tagNat_detVal (child_hashParent hp) (cost_ne_zero_of_hashParent hp) x hx
...
def tagging : graph.Tagging where
  ...
  tag_parent := by
    ...
    refine ⟨ps, hps, _, hf, hkp, fun x hx => ?_⟩
    have hps' : ps = (Name.parents p).map nameEquiv.toEmbedding := by
      rw [graph_kind_fin] at hkp
      cases h <;> simp only [hashParent, Option.some.injEq, reduceCtorEq] at hp <;> subst hp <;>
        exact (NodeKind.det.inj hkp).1.symm
    have hmem : (twOf h).fin ∈ ps := by
      rw [hps']
      exact Finset.mem_map_of_mem _ (twOf_mem_parents hp)
    exact tagOf_eq_some_of_tagNat
      (tagNat_cast_detVal (child_hashParent hp) (cost_ne_zero_of_hashParent hp) x
        (trunc_twOf_of_constAt hp x (hx _ hmem)) _)
...
theorem val_updHash_of_not_mem_deps (ξ : Rec) (s : Name) (b : BitVec 256) (n : Name)
  ...
  | tc k t => rw [val_tc, val_tc]
  | tg j => rw [val_tg, val_tg]
  | te l => rw [val_te, val_te]
  | tr => rw [val_tr, val_tr]
...
128 bits. -/
theorem trunc_tw_append {n : ℕ} (hn : 128 ≤ n) (a : BitVec 128) (x : BitVec n) :
...
theorem trunc_of_tw_append_eq {n : ℕ} (hn : 128 ≤ n) {a : BitVec 128} {x : BitVec n}
    {u : BitVec (128 + n)} (e : a ++ x = u) : trunc u = trunc x := by
  subst e
  exact trunc_tw_append hn a x

/-- A chain input: the payload is the low 128 bits. -/
theorem trunc_of_tw_eq {a : BitVec 128} {x : BitVec 128} {u : BitVec 256} (e : a ++ x = u) :
    trunc u = x :=
...
theorem trunc_of_tw_cat3_eq {a : BitVec 128} {x y z : BitVec 128} {u : BitVec 512}
...
theorem trunc_of_tw_cat7_eq {a : BitVec 128} {b : Fin 7 → BitVec 128} {u : BitVec 1024}
