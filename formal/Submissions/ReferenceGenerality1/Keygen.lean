import Submissions.ReferenceGenerality1.Master
...
/-- The node `w` has its value in `x` if it is a constant node (a deterministic node without
parents). -/
def ConstAt (x : G.Assignment) (w : Fin G.size) : Prop :=
  ∀ ps hps f hf, G.kind w = .det ps hps f hf → ps = ∅ → x w = f x
...
structure Tagging (G : Graph P) where
  ...
  /-- The parent of every hash node is a deterministic node whose output carries the tag of that
  hash node, as soon as its constant parents have their values. -/
  tag_parent : ∀ v p hp hl, G.kind v = .hash p hp hl →
    ∃ ps hps f hf, G.kind p = .det ps hps f hf ∧
      ∀ x, (∀ w ∈ ps, G.ConstAt x w) → tag ⟨G.len p, f x⟩ = some v
...
theorem tagOK_evalRec (T : G.Tagging) (ξ : G.Rec) : G.TagOK T (G.evalRec ξ) := by
  ...
  have hval : G.evalRec ξ p = f (G.evalRec ξ) := by
    ...
    exact h
  rw [hval]
  refine htag _ fun w _ ps' hps' f' hf' hk' _ => ?_
  have h := G.evalRec_apply ξ w
  rw [hk'] at h
  exact h
...
assignment `x`, an input carrying its tag; and every constant node not in `l` (so evaluated
...
theorem tagged_finRange (T : G.Tagging) (x : G.Assignment) :
  ...
  ⟨fun _ _ p _ _ _ hp => absurd (List.mem_finRange p) hp,
    fun w hw => absurd (List.mem_finRange w) hw⟩
...
theorem Tagged.update {T : G.Tagging} {a : Fin G.size} {l : List (Fin G.size)}
    {x : G.Assignment} (hpw : (a :: l).Pairwise (· < ·)) (h : G.Tagged T (a :: l) x)
    (u : BitVec (G.len a))
    (hu : ∀ ps hps f hf, G.kind a = .det ps hps f hf → u = f x) :
    G.Tagged T l (Function.update x a u) := by
  obtain ⟨h1, h2⟩ := h
  -- the constant nodes outside `l` have their values in the updated assignment
  have hc : ∀ w, w ∉ l → G.ConstAt (Function.update x a u) w := by
    intro w hw ps hps f hf hk hps0
    subst hps0
    have hfx : f (Function.update x a u) = f x := hf _ _ (fun w' hw' => by simp at hw')
    rw [hfx]
    by_cases hwa : w = a
    · subst hwa
      rw [Function.update_self]
      exact hu _ hps f hf hk
    · rw [Function.update_of_ne hwa]
      refine h2 w ?_ ∅ hps f hf hk rfl
  ...
  · subst hpa
    ...
    refine htag x fun w hw => ?_
    -- a parent of `p` precedes `p`, the head of the list, so it has been evaluated
    have hwp : w < p := hps w hw
    have hwl : w ∉ p :: l := by
      rw [List.pairwise_cons] at hpw
      simp only [List.mem_cons, not_or]
      exact ⟨ne_of_lt hwp, fun hwl => lt_asymm hwp (hpw.1 w hwl)⟩
    exact h2 w hwl
  · rw [Function.update_of_ne hpa]
    refine h1 v (List.mem_cons_of_mem a hv) p hp hl hk ?_
    simp only [List.mem_cons, not_or]
    exact ⟨hpa, hpl⟩
  ...
  | a :: l, hpw, x, c, hfresh, htagged => by
    ...
    · rw [G.evalNode_of_kind_eq_source x hk, run_map, run_pure, E_map, E_pure]
      rw [E_run_evalFold T z g l hpw _ c (Fresh.tail G hfresh)
        (Tagged.update G hpw' htagged _ fun _ _ _ _ h => by rw [hk] at h; cases h)]
    ...
    · rw [G.evalNode_of_kind_eq_det x hk, run_map, run_pure, E_map, E_pure]
      rw [E_run_evalFold T z g l hpw _ c (Fresh.tail G hfresh)
        (Tagged.update G hpw' htagged _ fun _ _ _ _ h => by
    ...
    · have hpa : p ∉ a :: l := by
      ...
      have htq : T.tag ⟨G.len p, x p⟩ = some a :=
        htagged.1 a (List.mem_cons_self ..) p hp hl hk hpa
