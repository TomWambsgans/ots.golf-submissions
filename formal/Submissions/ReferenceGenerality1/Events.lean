import Submissions.ReferenceGenerality1.Values
...
is the special case of a 128-bit tweak). -/
...
/-- The input of a chain hash: its tweak word, then the value before it. -/
theorem yv_ci (hy : graph.ReconEqs d (fins A) given y) {k : Fin 63} {t : Fin 14}
    (he : Evaluated A (ci k t)) :
    yv y (ci k t) = trunc (yv y (tc k t)) ++ trunc (yv y (prev k t)) := by
  rw [yv_det hy he rfl (by simp)]
  show (trunc (y (tc k t).fin) ++ trunc (y (prev k t).fin) : BitVec (128 + 128)) = _
  unfold yv
  rw [trunc_cast_eq, trunc_cast_eq]

theorem yv_gc (hy : graph.ReconEqs d (fins A) given y) {j : Fin 21}
    (he : Evaluated A (gc j)) :
    yv y (gc j) = trunc (yv y (tg j)) ++ cat3 (yv y (cv (chainOf j 0) 13))
      (yv y (cv (chainOf j 1) 13)) (yv y (cv (chainOf j 2) 13)) := by
  rw [yv_det hy he rfl (by simp)]
  show (trunc (y (tg j).fin) ++ cat3 (trunc (y _)) (trunc (y _)) (trunc (y _)) :
    BitVec (128 + 384)) = _
  rw [show trunc (y (tg j).fin) = trunc (yv y (tg j)) from (trunc_cast_eq _ _).symm]
  refine congrArg (fun a => trunc (yv y (tg j)) ++ a) ?_
  congr 1 <;> exact trunc_eq_cast (graph_len_fin _) _

theorem yv_ec (hy : graph.ReconEqs d (fins A) given y) {l : Fin 7}
    (he : Evaluated A (ec l)) :
    yv y (ec l) = trunc (yv y (te l)) ++ cat3 (yv y (gv (groupOf l 0)))
      (yv y (gv (groupOf l 1))) (yv y (gv (groupOf l 2))) := by
  rw [yv_det hy he rfl (by simp)]
  show (trunc (y (te l).fin) ++ cat3 (trunc (y _)) (trunc (y _)) (trunc (y _)) :
    BitVec (128 + 384)) = _
  rw [show trunc (y (te l).fin) = trunc (yv y (te l)) from (trunc_cast_eq _ _).symm]
  refine congrArg (fun a => trunc (yv y (te l)) ++ a) ?_
  congr 1 <;> exact trunc_eq_cast (graph_len_fin _) _

theorem yv_rc (hy : graph.ReconEqs d (fins A) given y) (he : Evaluated A rc) :
    yv y rc = trunc (yv y tr) ++ cat7 fun l => yv y (ev l) := by
  rw [yv_det hy he rfl (by simp)]
  show (trunc (y tr.fin) ++ cat7 (fun l => trunc (y (ev l).fin)) : BitVec (128 + 896)) = _
  rw [show trunc (y tr.fin) = trunc (yv y tr) from (trunc_cast_eq _ _).symm]
  exact congrArg (fun a => trunc (yv y tr) ++ cat7 a)
...
theorem yv_ci_ne (hy : graph.ReconEqs d (fins A) given y) {k : Fin 63} {t : Fin 14}
  ...
  exact hne (trunc_injective_of_len (Name.len_prev k t) (append_inj (n := 128) heq).2)
...
theorem child_twOf {h p : Name} (hp : hashParent h = some p) : child (twOf h) = some p := by
  cases h <;> simp only [hashParent, Option.some.injEq, reduceCtorEq] at hp <;> subst hp <;> rfl

/-- The tweak word of an evaluated hash node is evaluated: it is never revealed. -/
theorem evaluated_twOf (hA : IsCut A) {h p : Name} (hp : hashParent h = some p)
    (he : Evaluated A h) : Evaluated A (twOf h) := by
  have hpE := evaluated_hashParent hA hp he
  refine ⟨fun hm => ?_, fun m hm => ?_⟩
  · have := hA.notTw _ hm
    rw [isTw_twOf hp] at this
    exact Bool.noConfusion this
  · rw [above_of_child (child_twOf hp)] at hm
    rcases hm with rfl | hm
    · exact hpE.1
    · exact hpE.2 m hm
...
theorem tagNat_yv (hA : IsCut A) (hy : graph.ReconEqs d (fins A) given y) {h p : Name}
  ...
  refine tagNat_detVal_of_hashParent hp y ?_
  have hw := yv_det hy (evaluated_twOf hA hp he) (by
      cases h <;> simp only [hashParent, Option.some.injEq, reduceCtorEq] at hp <;> rfl)
    (by cases h <;> simp only [hashParent, Option.some.injEq, reduceCtorEq] at hp <;>
      exact fun k e => nomatch e)
  exact (trunc_cast_eq (graph_len_fin (twOf h)) (y (twOf h).fin)).symm.trans
    ((congrArg trunc hw).trans (detVal_twOf hp y))
...
theorem up {A : Finset Name} (hA : IsCut A) {ξ : Rec} {d : Cache paperParams}
  ...
  | _ n ih =>
    ...
    | tc k t =>
      have hch : child (tc k t) = some (ci k t) := rfl
      have hcE : Evaluated A (ci k t) :=
        ⟨hv _ (Above.child hch), fun m hm => hv m (Above.step hch hm)⟩
      refine ih' _ (by have := height_child hch; omega) hcE.2 rfl ?_
      intro heq
      rw [yv_ci hy hcE, val_ci] at heq
      exact hne (trunc_injective_of_len rfl (((append_inj (n := 128) heq).1).trans
        (by rw [val_tc]; exact (trunc_eq_self _).symm)))
    | tg j =>
      have hch : child (tg j) = some (gc j) := rfl
      have hcE : Evaluated A (gc j) :=
        ⟨hv _ (Above.child hch), fun m hm => hv m (Above.step hch hm)⟩
      refine ih' _ (by have := height_child hch; omega) hcE.2 rfl ?_
      intro heq
      rw [yv_gc hy hcE, val_gc'] at heq
      exact hne (trunc_injective_of_len rfl (((append_inj (n := 384) heq).1).trans
        (by rw [val_tg]; exact (trunc_eq_self _).symm)))
    | te l =>
      have hch : child (te l) = some (ec l) := rfl
      have hcE : Evaluated A (ec l) :=
        ⟨hv _ (Above.child hch), fun m hm => hv m (Above.step hch hm)⟩
      refine ih' _ (by have := height_child hch; omega) hcE.2 rfl ?_
      intro heq
      rw [yv_ec hy hcE, val_ec'] at heq
      exact hne (trunc_injective_of_len rfl (((append_inj (n := 384) heq).1).trans
        (by rw [val_te]; exact (trunc_eq_self _).symm)))
    | tr =>
      have hch : child tr = some rc := rfl
      have hcE : Evaluated A rc :=
        ⟨hv _ (Above.child hch), fun m hm => hv m (Above.step hch hm)⟩
      refine ih' _ (by have := height_child hch; omega) hcE.2 rfl ?_
      intro heq
      rw [yv_rc hy hcE, val_rc'] at heq
      exact hne (trunc_injective_of_len rfl (((append_inj (n := 896) heq).1).trans
        (by rw [val_tr]; exact (trunc_eq_self _).symm)))
...
theorem events_ne {A A' : Finset Name} (hA : IsCut A) (hA' : IsCut A')
  ...
  obtain ⟨h, hh⟩ :=
    Option.isSome_iff_exists.mp ((hashOf_isSome_iff v).mpr ⟨hlen, hns, hA.notTw v hvA⟩)
