...
def ancSet : Name → Finset Name
  ...
  | ci k t => (Finset.univ.filter fun t' : Fin 14 => t < t').image (ci k) ∪
      ...
      {gc ⟨k / 3, by omega⟩, gh ⟨k / 3, by omega⟩, gv ⟨k / 3, by omega⟩,
        ec ⟨k / 9, by omega⟩, eh ⟨k / 9, by omega⟩, ev ⟨k / 9, by omega⟩, rc, rh}
  | tc k t => insert (ci k t) ((Finset.univ.filter fun t' : Fin 14 => t < t').image (ci k) ∪
      (Finset.univ.filter fun t' : Fin 14 => t ≤ t').image (ch k) ∪
      (Finset.univ.filter fun t' : Fin 14 => t ≤ t').image (cv k) ∪
      {gc ⟨k / 3, by omega⟩, gh ⟨k / 3, by omega⟩, gv ⟨k / 3, by omega⟩,
        ec ⟨k / 9, by omega⟩, eh ⟨k / 9, by omega⟩, ev ⟨k / 9, by omega⟩, rc, rh})
  | tg j => insert (gc j) {gh j, gv j, ec ⟨j / 3, by omega⟩, eh ⟨j / 3, by omega⟩, ev ⟨j / 3, by omega⟩,
      rc, rh}
  | te l => insert (ec l) {eh l, ev l, rc, rh}
  | tr => insert rc {rh}
...
theorem ancSet_child {n p : Name} (h : child n = some p) : ancSet n = insert p (ancSet p) := by
  ...
  | tc k t => simp only [Name.child, Option.some.injEq] at h; subst h; rfl
  | tg j => simp only [Name.child, Option.some.injEq] at h; subst h; rfl
  | te l => simp only [Name.child, Option.some.injEq] at h; subst h; rfl
  | tr => simp only [Name.child, Option.some.injEq] at h; subst h; rfl
...
theorem hashOf_isSome_iff (a : Name) :
    (hashOf a).isSome ↔ a.len = 128 ∧ (∀ k, a ≠ src k) ∧ a.isTw = false := by
  cases a <;> simp [hashOf, Name.len, Name.isTw]
...
/-- A cut: an antichain of 128-bit nodes, other than the root and the tweak words, meeting every
source path. -/
structure IsCut (A : Finset Name) : Prop where
  values : ∀ n ∈ A, n.len = 128
  notTw : ∀ n ∈ A, n.isTw = false
...
`Finset.univ : Finset Name` through the `Fintype` instance (3706 elements), which exhausts the
...
theorem exists_mem_evaluated_of_ne {A A' : Finset Name} (hA : IsCut A) (hA' : IsCut A')
  ...
  have hsome : (hashOf a).isSome :=
    (hashOf_isSome_iff a).mpr ⟨hA'.values a haA', hns, hA'.notTw a haA'⟩
