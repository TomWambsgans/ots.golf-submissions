...
import Submissions.ReferenceGenerality1.Count
...
theorem mem_cutOf_isTw {c : Choice} {n : Name} (hn : n ∈ cutOf c) : n.isTw = false := by
  rw [mem_cutOf_iff] at hn
  rcases hn with ⟨l, _, rfl⟩ | ⟨j, _, rfl⟩ | ⟨k, _, rfl⟩
  · rfl
  · rfl
  · unfold chainNode
    split_ifs <;> rfl
