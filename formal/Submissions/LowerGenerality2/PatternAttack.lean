...
def targets (i : Fin F.numSets) : Finset ℕ := (S.samePattern i).image Fin.val

def fallback (m : Message P) : Message P × Signature F :=
  (m, 0, List.replicate ((P.signatureBits - F.nonceBits) + 1) false)

def fromResult (i : Fin F.numSets) (xa xr : S.graph.Assignment) (m : Message P) :
    Option (Nonce F × ℕ) → Message P × Signature F
  | none => fallback m
  | some (η, n) =>
      if hn : n < F.numSets then
        (m, η, Conversion.convert S i ⟨n, hn⟩ xa xr)
      else fallback m

def forgeFrom (T : ℕ) (i : Fin F.numSets) (xa xr : S.graph.Assignment)
    (m₁ : Message P) : OracleComp (Spec P) (Message P × Signature F) := do
  let m₂ ← sampleBits P P.msgBits
  if m₂ = m₁ then pure (fallback m₁) else
    fromResult S i xa xr m₂ <$> PatternSearch.search P F (targets S i) m₂ T 0

def forge (T : ℕ) (m₁ : Message P) :
    Option (Signature F) → OracleComp (Spec P) (Message P × Signature F)
  | none => pure (fallback m₁)
  | some (η₁, pl) => do
      let n ← index P F m₁ η₁
      if hn : n < F.numSets then
        let i : Fin F.numSets := ⟨n, hn⟩
        let xa := S.graph.decode (S.sets i) pl
        let xr ← S.graph.reconstruct (S.sets i) xa
        forgeFrom S T i xa xr m₁
      else pure (fallback m₁)

def adversary (T : ℕ) : Adversary P F where
  State := Message P
  choose _ := (fun m => (m,m)) <$> sampleBits P P.msgBits
  forge := forge S T

/-- The final weak-security check after a signature was returned. -/
def check (pk : PublicKey P) (m₁ : Message P) (out : Message P × Signature F) :
...
theorem mem_targets (i : Fin F.numSets) (n : ℕ) (hn : n ∈ targets S i) :
    ∃ j : Fin F.numSets, j.val = n ∧ S.graph.evalHash (S.sets j) = S.graph.evalHash (S.sets i) := by
  obtain ⟨j, hj, rfl⟩ := Finset.mem_image.mp hn
  refine ⟨j,rfl, (S.hashPattern_eq_iff j i).mp ?_⟩
  exact (Finset.mem_filter.mp hj).2

theorem card_targets (i : Fin F.numSets) : (targets S i).card = (S.samePattern i).card :=
...
theorem check_fromResult (i : Fin F.numSets) (x : S.graph.Assignment)
    (m₁ m₂ : Message P) (hne : m₂ ≠ m₁) (c : Cache P)
    (hc : S.graph.CacheConsistent x c) (η : Nonce F) (n : ℕ) (hn : n ∈ targets S i)
    (w : BitVec P.hashBits) (hw : c ⟨P.msgBits + F.nonceBits,m₂ ++ η⟩ = some w)
    (hidx : (w.setWidth F.idxBits).toNat = n) :
...
theorem cost_forgeFrom (T : ℕ) (i : Fin F.numSets) (xa xr : S.graph.Assignment)
    (m₁ : Message P) (hidx : blockCost P (P.msgBits + F.nonceBits) = 1) :
...
theorem cost_forge (T : ℕ) (hidx : blockCost P (P.msgBits + F.nonceBits) = 1)
...
theorem search_check_ge (T : ℕ) (i : Fin F.numSets) (x : S.graph.Assignment)
    (m₁ m₂ : Message P) (hne : m₂ ≠ m₁) (c : Cache P)
    (hc : S.graph.CacheConsistent x c) :
    E (run P (PatternSearch.search P F (targets S i) m₂ T 0) c)
      (fun p => if p.1.isSome then 1 else 0) ≤
    E (run P (PatternSearch.search P F (targets S i) m₂ T 0) c) (fun p =>
...
theorem forgeFrom_success_ge (S : Scheme paperParams paperDagFormat) (i : Fin paperDagFormat.numSets)
  ...
  refine le_trans ?_ (BareLower.expectedValue_ge_indicator ($ᵗ BitVec paperParams.msgBits)
    (fun m₂ => BareLower.FreshMessage paperDagFormat c m₂ ∧ m₂ ≠ m₁) _ (1 / 9) ?_)
