...
def goodIndices (S : Scheme paperParams paperDagFormat) : Finset ℕ :=
  (Finset.univ.filter fun i => 8 ≤ (S.samePattern i).card).image Fin.val

theorem mem_goodIndices (S : Scheme paperParams paperDagFormat) (i : Fin paperDagFormat.numSets) :
...
theorem goodIndices_lt (S : Scheme paperParams paperDagFormat) :
    ∀ n ∈ goodIndices S, n < paperDagFormat.numSets := by
  intro n hn
  obtain ⟨i, _, rfl⟩ := Finset.mem_image.mp hn
  exact i.isLt

theorem card_goodIndices (S : Scheme paperParams paperDagFormat) (hcost : ∀ i, S.verifyCost i ≤ 17) :
    3 * paperDagFormat.numSets ≤ 4 * (goodIndices S).card := by
  have hb := S.card_small_samePattern_mul_four_le hcost
  have hs := Finset.card_filter_add_card_filter_not
    (s := (Finset.univ : Finset (Fin paperDagFormat.numSets)))
...
theorem cost_signIdx {P : Params} {F : DagFormat} (S : Scheme P F)
    (hidx : blockCost P (P.msgBits + F.nonceBits) = 1) (m : Message P) :
    CostAtMost P (signIdx P F m) F.trialLimit := by
