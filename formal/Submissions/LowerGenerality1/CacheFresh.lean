...
abbrev CacheP (P : Params) := (hashSpec P).QueryCache

/-- `D` contains every query for which the cache already has an answer. -/
def HasSupport {P : Params} (c : CacheP P) (D : Finset Query) : Prop :=
  ∀ q, (c q).isSome → q ∈ D
...
theorem HasSupport.cacheQuery {P : Params} {c : CacheP P} {D : Finset Query}
    (hc : HasSupport c D) (q : Query) (u : BitVec P.hashBits) :
...
theorem exists_support_run {P : Params} {α : Type} (oa : OracleComp (Spec P) α) :
    ∀ {b : ℕ}, CostAtMost P oa b → ∀ {c : CacheP P} {D : Finset Query},
      HasSupport c D → ∀ p ∈ support (run P oa c),
  ...
  | query_bind t k ih =>
    ...
    · have hqcost : 1 ≤ queryCost P (.inr q) := Nat.le_max_left _ _
      rcases hcache : c q with _ | v
      · rw [oracleImpl_run_inr_none P hcache, support_bind] at hu
        simp only [Set.mem_iUnion] at hu
        obtain ⟨w, _, hw⟩ := hu
        simp only [support_pure, Set.mem_singleton_iff, Prod.mk.injEq] at hw
        obtain ⟨rfl, rfl⟩ := hw
        ...
        omega
      · rw [oracleImpl_run_inr_some P hcache, support_pure] at hu
...
theorem exists_support_run_empty {P : Params} {α : Type} {oa : OracleComp (Spec P) α}
    {b : ℕ} (hb : CostAtMost P oa b) (p : α × CacheP P)
    (hp : p ∈ support (run P oa ∅)) :
    ∃ D : Finset Query, HasSupport p.2 D ∧ D.card ≤ b := by
  simpa only [Finset.card_empty, Nat.zero_add] using
    exists_support_run oa hb (hasSupport_empty P) p hp
