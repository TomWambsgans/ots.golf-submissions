...
theorem run_evalNode_cached (x : S.graph.Assignment) (v : Fin S.graph.size)
    (s : BitVec (S.graph.len v)) (a : BitVec P.hashBits) (c : Cache P)
    (hc : (S.graph.kind v).IsHash →
      c ⟨(S.graph.kind v).inLen, (S.graph.kind v).input x⟩ = some a) :
    run P (S.graph.evalNode x v (pure s)) c =
      pure ((S.graph.kind v).value x s a, c) := by
  unfold Graph.evalNode
  generalize hk : S.graph.kind v = k at hc ⊢
  cases k with
  | source => exact run_pure P s c
  | det ps hps f hf => exact run_pure P (f x) c
  | hash p hp hl =>
    have hh := hc trivial
    simp only [NodeKind.inLen, NodeKind.input] at hh
    rw [run_map]
    have hr : run P (hash P (x p)) c = pure (a, c) := by
      unfold hash run
      rw [simulateQ_spec_query, oracleImpl_run_inr_some P hh]
    rw [hr]
    rfl
...
theorem run_reconstruct_cached (tg : S.graph.Tab) (A : Finset (Fin S.graph.size))
    (given : S.graph.Assignment) (c : Cache P)
    ...
    run P (S.graph.reconstruct A given) c = pure (S.graph.reconTab tg A given, c) := by
  let y := S.graph.reconTab tg A given
  let step : S.graph.Assignment → Fin S.graph.size → OracleComp (Spec P) S.graph.Assignment :=
    fun x v =>
  ...
  have hstep : ∀ x v, (∀ w ∈ (S.graph.kind v).parents, x w = y w) →
      run P (step x v) c = pure (Function.update x v (y v), c) := by
    intro x v hparents
    dsimp only [step]
    by_cases hA : v ∈ A
  ...
  have hfold : ∀ (l : List (Fin S.graph.size)), l.Pairwise (· < ·) →
      ∀ (x : S.graph.Assignment), (∀ w, w ∉ l → x w = y w) →
        run P (l.foldlM step x) c = pure (y,c) := by
    intro l hl
    induction l with
    | nil =>
      intro x hx
...
theorem run_reconstruct_encode_cached (tg : S.graph.Tab) (j : Fin F.numSets)
    ...
    run P (S.graph.reconstruct (S.sets j)
...
theorem run_convert_reconstruct (i j : Fin F.numSets) (z : S.graph.Assignment)
    ...
    let xa := S.graph.decode (S.sets i) (S.graph.encode (S.sets i) (S.graph.evalTab z tg))
    let xr := S.graph.reconTab tg (S.sets i) xa
    run P (S.graph.reconstruct (S.sets j) (S.graph.decode (S.sets j) (convert S i j xa xr))) c =
...
theorem run_index_cached (m : Message P) (η : Nonce F) (c : Cache P)
  ...
  have hh : run P (hash P (m ++ η)) c = pure (w,c) := by
    unfold hash run
    rw [simulateQ_spec_query, oracleImpl_run_inr_some P hc]
  rw [hh]
  rfl
...
def cacheTab (c : Cache P) : S.graph.Tab :=
  fun v u => (c ⟨(S.graph.kind v).inLen, u⟩).getD 0

theorem evalTab_cacheTab (x : S.graph.Assignment) (c : Cache P)
...
theorem cacheTab_cached (x : S.graph.Assignment) (c : Cache P)
