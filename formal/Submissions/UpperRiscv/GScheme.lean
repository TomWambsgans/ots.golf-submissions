...
scheme of this root accepts an index when it lies in `validSet F` (its 32 nibbles are at most 14
...
indices. -/
structure GScheme (P : Params) (F : DagFormat) where
  /-- The public computation. -/
  graph : Graph P
  /-- The disclosure sets. -/
  sets : Idx F → Finset (Fin graph.size)
...
def verifyCost (i : Idx F) : ℕ := idxCost P F + S.graph.reconstructCost (S.sets i)
...
def experiment (A : Adversary P F) : OracleComp (Spec P) Bool := do
  let (pk, sk) ← S.keygen
...
def Secure : Prop :=
  ∀ (A : Adversary P F) (B : ℕ), CostAtMost P (S.experiment A) B →
