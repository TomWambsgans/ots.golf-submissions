...
def sigOf (ξ : Rec) (r : Option (Nonce paperDagFormat × Idx paperDagFormat)) : Option (Signature paperDagFormat) :=
  r.map fun r => (r.1, graph.encode (forestScheme.sets r.2) (graph.evalRec ξ))

/-- The disclosure set of the outcome of the signing loop. -/
def cutOf? (r : Option (Nonce paperDagFormat × Idx paperDagFormat)) : Option (Finset Name) :=
  r.map fun r => setsName r.2

/-- The index of the outcome of the signing loop. -/
def idxOf? (r : Option (Nonce paperDagFormat × Idx paperDagFormat)) : Option ℕ := r.map fun r => r.2.val
