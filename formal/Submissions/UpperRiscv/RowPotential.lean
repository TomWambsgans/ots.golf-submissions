...
structure RowHyp (P : Params) : Prop where
  ...
  two_le : 2 ≤ numValid P
  numSets_le : 2 * numValid P ≤ 2 ^ P.idxBits
...
def gCls (d : Cache P) (m₀ : Message P) (i : ℕ) : ℝ :=
  if i ∈ validSet P then
    (if i ∈ V P d then (1 - Row.r P d) / Row.D P d m₀ + ∑ m, Row.hit P d m i / Row.D P d m
    else 1 / Row.M P)
  else
...
theorem M_pos (hP : RowHyp P) : 0 < M P := by
  unfold M; have := hP.two_le; exact_mod_cast (by omega : 0 < numValid P)
...
theorem v_le_M (d : Cache P) : v P d ≤ M P := by
  unfold v M; exact_mod_cast card_V_le_numValid P d
...
theorem psi_step :
  ...
  have ha : ∀ m, Row.a P d' m = Row.a P d m + if m = m₀ ∧ i ∈ validSet P then 1 else 0 :=
    fun m => by simp only [Row.a, hd', rowAcc_cacheQuery w hfresh m]; push_cast; rfl
  have hv : Row.v P d' = Row.v P d + if i ∈ validSet P ∧ i ∉ V P d then 1 else 0 := by
  ...
  have hb : ∀ m, Row.b P d' m ≤
    ...
    exact_mod_cast this
  have hD : ∀ m, Row.D P d' m = Row.D P d m + (if m = m₀ ∧ i ∈ validSet P then 1 else 0) -
  ...
  · -- an index already held
    have hlt : i ∈ validSet P := h1
    have hr : Row.r P d' = Row.r P d := by
      simp only [Row.r, hv, if_neg (fun h : i ∈ validSet P ∧ i ∉ V P d => h.2 h2), add_zero]
  ...
  · -- a new accepted index
    ...
    have hr : Row.r P d' = Row.r P d + 1 / Row.M P := by
      simp only [Row.r, hv, if_pos (⟨hlt, hnV⟩ : i ∈ validSet P ∧ i ∉ V P d)]
      ring
  ...
  · -- a rejected index
    have hnV : i ∉ V P d := fun h => h1 (V_sub_valid P d i h)
    have hr : Row.r P d' = Row.r P d := by
      simp only [Row.r, hv, if_neg (fun h : i ∈ validSet P ∧ i ∉ V P d => h1 h.1), add_zero]
    have hrow : ∀ m, Row.pr P d' m ≤ Row.pr P d m + if m = m₀ then
      ...
      have hx : Row.x P d' m ≤ Row.x P d m := by
        simp only [Row.x, hr, ha m, if_neg (fun h : m = m₀ ∧ i ∈ validSet P => h1 h.2),
          add_zero]
        simp only [hhit, if_neg (fun h : m = m₀ ∧ i ∈ V P d => hnV h.2), add_zero] at hbm
        linarith
      by_cases hm : m = m₀
...
theorem sum_gCls_le (m₀ : Message P) :
  ...
  have hVsub : V P d ⊆ validSet P := V_sub_valid P d
  have hvalid : validSet P ⊆ Finset.range (2 ^ P.idxBits) := fun i hi =>
  ...
  have hF1 : (Finset.range (2 ^ P.idxBits)).filter (fun i => ¬ i ∈ validSet P) =
      Finset.range (2 ^ P.idxBits) \ validSet P := by
    ext i; simp only [Finset.mem_filter, Finset.mem_sdiff]
  have hF2 : ((Finset.range (2 ^ P.idxBits)).filter (fun i => i ∈ validSet P)).filter
  ...
  have hF3 : ((Finset.range (2 ^ P.idxBits)).filter (fun i => i ∈ validSet P)).filter
      (fun i => ¬ i ∈ V P d) = validSet P \ V P d := by
    ext i
  ...
  have hdd : ∀ m, Row.dd P d m = ∑ i ∈ V P d, Row.hit P d m i := fun m => by
    ...
    push_cast
    rfl
  have e1 : ∑ _i ∈ Finset.range (2 ^ P.idxBits) \ validSet P, A = (Row.I P - Row.M P) * A := by
    rw [Finset.sum_const, Finset.card_sdiff_of_subset hvalid, Finset.card_range, nsmul_eq_mul,
      Nat.cast_sub (show (validSet P).card ≤ 2 ^ P.idxBits from numValid_le P)]
  ...
  have e3 : ∑ _i ∈ validSet P \ V P d, 1 / Row.M P = 1 - Row.r P d := by
    rw [Finset.sum_const, Finset.card_sdiff_of_subset hVsub, nsmul_eq_mul,
      Nat.cast_sub (show (V P d).card ≤ (validSet P).card from card_V_le_numValid P d)]
    simp only [Row.r, Row.v, Row.M, numValid]
    have : ((validSet P).card : ℝ) ≠ 0 := hM.ne'
    field_simp
...
theorem psi_dom (m : Message P) (c : ℕ) (hc1 : (rowFresh P d m).card ≤ c + P.trialLimit)
    ...
    ((rowBad P d m).card : ℝ≥0∞) + c * (((V P d).card : ℝ≥0∞) / 2 ^ P.idxBits) ≤
      ENNReal.ofReal (Row.θ P * psi P d) *
        ((rowAcc P d m).card + c * ((numValid P : ℝ≥0∞) / 2 ^ P.idxBits)) := by
  ...
  have hreal : Row.b P d m + c * (Row.v P d / 2 ^ P.idxBits) ≤
      Row.θ P * psi P d * (Row.a P d m + c * ((numValid P : ℝ) / 2 ^ P.idxBits)) := by
    have hv : Row.v P d / 2 ^ P.idxBits = Row.r P d * Row.q P := by
      have : (numValid P : ℝ) ≠ 0 := hM.ne'
      simp only [Row.r, Row.q, Row.I, Row.M]
      field_simp
    have hq' : (numValid P : ℝ) / 2 ^ P.idxBits = Row.q P := rfl
    rw [hv, hq']
  ...
  have hR : ENNReal.ofReal (Row.θ P * psi P d) *
      ((rowAcc P d m).card + c * ((numValid P : ℝ≥0∞) / 2 ^ P.idxBits)) =
      ENNReal.ofReal (Row.θ P * psi P d *
        (((rowAcc P d m).card : ℝ) + c * ((numValid P : ℝ) / 2 ^ P.idxBits))) := by
