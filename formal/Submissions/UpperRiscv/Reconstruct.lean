...
theorem verify_support {P : Params} {F : DagFormat} (S : GScheme P F) (pk : PublicKey P) (m : Message P)
    (σ : Signature F) (c : Cache P) :
    ∀ p ∈ support (run P (S.verify pk m σ) c),
      Cache.Sub c p.2 ∧ (p.1 = true →
        ∃ w, p.2 ⟨P.msgBits + F.nonceBits, m ++ σ.1⟩ = some w ∧
          ∃ hi : (w.setWidth F.idxBits).toNat ∈ validSet F,
  ...
  dsimp only at hp hw
  by_cases hi : (w.setWidth F.idxBits).toNat ∈ validSet F
  · rw [dif_pos hi] at hp
