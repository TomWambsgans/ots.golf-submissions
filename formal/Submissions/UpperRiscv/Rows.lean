...
theorem mem_V_iff {d : Cache P} {i : ℕ} :
    i ∈ V P d ↔ ∃ u w, d (encQuery P u) = some w ∧ idxOf P w ∈ validSet P ∧ idxOf P w = i := by
  constructor
  · intro h
...
theorem V_sub_valid (d : Cache P) : ∀ i ∈ V P d, i ∈ validSet P := by
  intro i hi
...
theorem rowAcc_cacheQuery (m : Message P) :
    (rowAcc P (d.cacheQuery (encQuery P (m₀ ++ η₀)) w) m).card =
      (rowAcc P d m).card + if m = m₀ ∧ idxOf P w ∈ validSet P then 1 else 0 := by
  by_cases hm : m = m₀ ∧ idxOf P w ∈ validSet P
  · obtain ⟨rfl, hi⟩ := hm
    rw [if_pos ⟨rfl, hi⟩]
...
theorem V_cacheQuery :
    (V P (d.cacheQuery (encQuery P (m₀ ++ η₀)) w)).card =
      (V P d).card + if idxOf P w ∈ validSet P ∧ idxOf P w ∉ V P d then 1 else 0 := by
  have hmem : ∀ i, i ∈ V P (d.cacheQuery (encQuery P (m₀ ++ η₀)) w) ↔
      i ∈ V P d ∨ (idxOf P w ∈ validSet P ∧ idxOf P w = i) := by
    intro i
