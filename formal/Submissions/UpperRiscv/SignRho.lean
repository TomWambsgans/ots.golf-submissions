...
theorem frac_acc (hidx : F.idxBits ≤ P.hashBits) (hM : numValid F ≤ 2 ^ F.idxBits) :
    ∑ w : BitVec P.hashBits, (Fintype.card (BitVec P.hashBits) : ℝ≥0∞)⁻¹ *
      (if idxOf P F w ∈ validSet F then (1 : ℝ≥0∞) else 0) = (numValid F : ℝ≥0∞) / 2 ^ F.idxBits := by
  rw [← Finset.mul_sum, Finset.sum_boole, card_idxOf_mem P F hidx _ (fun n hn => mem_validSet_lt hn),
    Nat.cast_mul, inv_card_mul_pow P F hidx]
  rfl

theorem frac_split (w : BitVec P.hashBits) :
    (if idxOf P F w ∈ validSet F then (1 : ℝ≥0∞) else 0) +
      (if idxOf P F w ∈ validSet F then (0 : ℝ≥0∞) else 1) = 1 := by
  split_ifs <;> simp
...
theorem ind_le_rowBad {d : Cache P} {m : Message P} {η : Nonce F} {w : BitVec P.hashBits}
    (hi : idxOf P F w ∈ validSet F) (hw : d (encQuery P F (m ++ η)) = some w) :
...
theorem signRhoLoop_bound (hM' : numValid F ≤ 2 ^ F.idxBits) (hidx : F.idxBits ≤ P.hashBits)
    (m : Message P) (d : Cache P) {β J : Type} [Nonempty J]
    (kont : J → Option (Nonce F × Idx F) → OracleComp (Spec P) β)
    (Fn : Option (Nonce F × Idx F) → Cache P → ℝ≥0∞)
    ...
    (hρ : ∀ c : ℕ, (rowFresh P F d m).card ≤ c + F.trialLimit → c ≤ (rowFresh P F d m).card →
      ((rowBad P F d m).card : ℝ≥0∞) + c * (((V P F d).card : ℝ≥0∞) / 2 ^ F.idxBits) ≤
        ρ * ((rowAcc P F d m).card + c * ((numValid F : ℝ≥0∞) / 2 ^ F.idxBits))) :
  ...
  have hnone : ∀ (d' : Cache P) (b : ℕ) (tried : Finset (Nonce F)), Cache.Sub d d' →
      ...
      Fn none d' + (if (none : Option (Nonce F × Idx F)) = none then lam * ρ else 0) ≤
        Φ d + κ * b + lam * ρ := by
  ...
  | succ k ih =>
    ...
    · simp only [signIdxLoop_succ P F m k tried hc] at hB ⊢
      ...
      set af : ℝ≥0∞ := (numValid F : ℝ≥0∞) / 2 ^ F.idxBits with haf
      set rf : ℝ≥0∞ := ∑ w : BitVec P.hashBits, (Fintype.card (BitVec P.hashBits) : ℝ≥0∞)⁻¹ *
        (if idxOf P F w ∈ validSet F then (0 : ℝ≥0∞) else 1) with hrf
        ...
        · -- fresh encoding query
          ...
          have perw : ∀ w : BitVec P.hashBits,
              ...
              Φ d + κ * b + lam * ((if idxOf P F w ∈ V P F d then 1 else 0) +
                (if idxOf P F w ∈ validSet F then 0 else ρ)) := by
            intro w
            by_cases hi : idxOf P F w ∈ validSet F
          ...
          calc ∑ w : BitVec P.hashBits, (Fintype.card (BitVec P.hashBits) : ℝ≥0∞)⁻¹ *
              ...
              ≤ ∑ w : BitVec P.hashBits, (Fintype.card (BitVec P.hashBits) : ℝ≥0∞)⁻¹ *
                  (Φ d + κ * b + lam * ((if idxOf P F w ∈ V P F d then 1 else 0) +
                    (if idxOf P F w ∈ validSet F then 0 else ρ))) :=
                Finset.sum_le_sum fun w _ => mul_le_mul_right (perw w) _
            _ = Φ d + κ * b + lam * (vf + ρ * rf) := by
                have hρw : ∀ w : BitVec P.hashBits, (if idxOf P F w ∈ validSet F then 0 else ρ) =
                    ρ * (if idxOf P F w ∈ validSet F then (0 : ℝ≥0∞) else 1) := fun w => by
                  split_ifs <;> simp
...
theorem signRho_bound (hM' : numValid F ≤ 2 ^ F.idxBits) (hidx : F.idxBits ≤ P.hashBits)
    (m : Message P) (d : Cache P) {β J : Type} [Nonempty J]
    (k : J → Option (Nonce F × Idx F) → OracleComp (Spec P) β)
    (Fn : Option (Nonce F × Idx F) → Cache P → ℝ≥0∞)
    (Φ : Cache P → ℝ≥0∞) (hΦ : EncInvariant P F Φ) (κ lam ρ : ℝ≥0∞)
    (I : Cache P → ℕ → Prop)
    (hI_fresh : ∀ c b q, I c b → c q = none → queryCost P (.inr q) ≤ b →
