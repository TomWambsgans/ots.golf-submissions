...
def rowAcc (d : Cache P) (m : Message P) : Finset (Nonce P) :=
  Finset.univ.filter fun η => ∃ w, d (encQuery P (m ++ η)) = some w ∧ idxOf P w ∈ validSet P
...
theorem frac_acc (hidx : P.idxBits ≤ P.hashBits) (hM : numValid P ≤ 2 ^ P.idxBits) :
    ∑ w : BitVec P.hashBits, (Fintype.card (BitVec P.hashBits) : ℝ≥0∞)⁻¹ *
      (if idxOf P w ∈ validSet P then (1 : ℝ≥0∞) else 0) = (numValid P : ℝ≥0∞) / 2 ^ P.idxBits := by
  rw [← Finset.mul_sum, Finset.sum_boole, card_idxOf_mem P hidx _ (fun n hn => mem_validSet_lt hn),
    Nat.cast_mul, inv_card_mul_pow P hidx]
  rfl

theorem frac_split (w : BitVec P.hashBits) :
    (if idxOf P w ∈ validSet P then (1 : ℝ≥0∞) else 0) +
      (if idxOf P w ∈ validSet P then (0 : ℝ≥0∞) else 1) = 1 := by
  split_ifs <;> simp
...
theorem ind_le_rowBad {d : Cache P} {m : Message P} {η : Nonce P} {w : BitVec P.hashBits}
    (hi : idxOf P w ∈ validSet P) (hw : d (encQuery P (m ++ η)) = some w) :
...
theorem signRhoLoop_bound (hM' : numValid P ≤ 2 ^ P.idxBits) (hidx : P.idxBits ≤ P.hashBits)
    (m : Message P) (d : Cache P) {β J : Type} [Nonempty J]
    (kont : J → Option (Nonce P × Idx P) → OracleComp (Spec P) β)
    (F : Option (Nonce P × Idx P) → Cache P → ℝ≥0∞)
    ...
    (hρ : ∀ c : ℕ, (rowFresh P d m).card ≤ c + P.trialLimit → c ≤ (rowFresh P d m).card →
      ((rowBad P d m).card : ℝ≥0∞) + c * (((V P d).card : ℝ≥0∞) / 2 ^ P.idxBits) ≤
        ρ * ((rowAcc P d m).card + c * ((numValid P : ℝ≥0∞) / 2 ^ P.idxBits))) :
  ...
  have hnone : ∀ (d' : Cache P) (b : ℕ) (tried : Finset (Nonce P)), Cache.Sub d d' →
      ...
      F none d' + (if (none : Option (Nonce P × Idx P)) = none then lam * ρ else 0) ≤
        Φ d + κ * b + lam * ρ := by
  ...
  | succ k ih =>
    ...
    · simp only [signIdxLoop_succ P m k tried hc] at hB ⊢
      ...
      set af : ℝ≥0∞ := (numValid P : ℝ≥0∞) / 2 ^ P.idxBits with haf
      set rf : ℝ≥0∞ := ∑ w : BitVec P.hashBits, (Fintype.card (BitVec P.hashBits) : ℝ≥0∞)⁻¹ *
        (if idxOf P w ∈ validSet P then (0 : ℝ≥0∞) else 1) with hrf
        ...
        · -- fresh encoding query
          ...
          have perw : ∀ w : BitVec P.hashBits,
              ...
              Φ d + κ * b + lam * ((if idxOf P w ∈ V P d then 1 else 0) +
                (if idxOf P w ∈ validSet P then 0 else ρ)) := by
            intro w
            by_cases hi : idxOf P w ∈ validSet P
          ...
          calc ∑ w : BitVec P.hashBits, (Fintype.card (BitVec P.hashBits) : ℝ≥0∞)⁻¹ *
              ...
              ≤ ∑ w : BitVec P.hashBits, (Fintype.card (BitVec P.hashBits) : ℝ≥0∞)⁻¹ *
                  (Φ d + κ * b + lam * ((if idxOf P w ∈ V P d then 1 else 0) +
                    (if idxOf P w ∈ validSet P then 0 else ρ))) :=
                Finset.sum_le_sum fun w _ => mul_le_mul_right (perw w) _
            _ = Φ d + κ * b + lam * (vf + ρ * rf) := by
                have hρw : ∀ w : BitVec P.hashBits, (if idxOf P w ∈ validSet P then 0 else ρ) =
                    ρ * (if idxOf P w ∈ validSet P then (0 : ℝ≥0∞) else 1) := fun w => by
                  split_ifs <;> simp
...
theorem signRho_bound (hM' : numValid P ≤ 2 ^ P.idxBits) (hidx : P.idxBits ≤ P.hashBits)
    (m : Message P) (d : Cache P) {β J : Type} [Nonempty J]
    (k : J → Option (Nonce P × Idx P) → OracleComp (Spec P) β)
    (F : Option (Nonce P × Idx P) → Cache P → ℝ≥0∞)
    (Φ : Cache P → ℝ≥0∞) (hΦ : EncInvariant P Φ) (κ lam ρ : ℝ≥0∞)
    (I : Cache P → ℕ → Prop)
    (hI_fresh : ∀ c b q, I c b → c q = none → queryCost P (.inr q) ≤ b →
