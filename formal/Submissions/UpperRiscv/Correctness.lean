...
theorem sign_result (S : GScheme P F) (x : S.graph.Assignment) (m : Message P)
    (σ : Signature F) (c d : Cache P)
    (h : (some σ, d) ∈ support (run P (S.sign x m) c)) :
    ∃ i : Idx F, ∃ w : BitVec P.hashBits,
...
theorem verify_accepts (S : GScheme P F) (x : S.graph.Assignment) (m : Message P)
...
theorem correct (S : GScheme P F) : S.toAlgorithm.Correct := by
  intro message
