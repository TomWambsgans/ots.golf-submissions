...
def FA (pk : BitVec 128) (x : Message paperParams × A.State) (d : Cache paperParams) : ℝ≥0∞ :=
...
theorem stageA_cont (pk : BitVec 128) (x : Message paperParams × A.State) (d : Cache paperParams) (b' : ℕ)
    (hI : Inv d b') (hB : ∀ ξ ∈ fiberA pk, CostAtMost paperParams (rest₂ A pk (graph.evalRec ξ) x) b') :
  ...
  have hsig : E (run paperParams (signIdx paperParams paperDagFormat x.1) d) (fun p => ∑ ξ ∈ T, w *
    ...
    refine signRho_bound paperParams paperDagFormat (numValid_le paperDagFormat) (by decide) x.1 d
...
theorem stageA_master (pk : BitVec 128) (b : ℕ) (hb : b ≤ 2 ^ 127)
    (hB : ∀ ξ ∈ fiberA pk, CostAtMost paperParams (A.choose pk >>= rest₂ A pk (graph.evalRec ξ)) b) :
    E (run paperParams (A.choose pk) ∅) (fun p => FA A pk p.1 p.2) ≤ κ * sumW (fiberA pk) * b := by
  have hne : Nonempty {ξ // ξ ∈ fiberA pk} := (fiberA_nonempty pk).to_subtype
  have hF : ∀ (x : Message paperParams × A.State) (d : Cache paperParams) (b' : ℕ), Inv d b' →
      (∀ j : {ξ // ξ ∈ fiberA pk}, CostAtMost paperParams (rest₂ A pk (graph.evalRec j.1) x) b') →
      FA A pk x d ≤ ΦA pk d + κ * sumW (fiberA pk) * b' := by
    intro x d b' hI hB'
  ...
  have hB0 : ∀ j : {ξ // ξ ∈ fiberA pk},
    ...
    fun j => hB j.1 j.2
  have h := master_family paperParams (α := Message paperParams × A.State) (β := Bool)
...
theorem stageA_iub (ξ : Rec) :
    E (run paperParams (rest A (pkOf ξ, graph.evalRec ξ)) (kc ξ)) g ≤
      E (run paperParams (A.choose (pkOf ξ)) ∅) (fun p => if Cache.Hits p.2 (kc ξ) then 1 else
        E (run paperParams (rest₂ A (pkOf ξ) (graph.evalRec ξ) p.1) (Cache.extend p.2 (kc ξ))) g) := by
  unfold rest
  dsimp only
  rw [run_bind, E_bind]
  have hdisj : Cache.Disjoint ∅ (kc ξ) := fun _ _ => rfl
  have h := iub paperParams (A.choose (pkOf ξ)) (kc ξ)
    (fun p => E (run paperParams (rest₂ A (pkOf ξ) (graph.evalRec ξ) p.1) p.2) g)
    (fun p => E_le_one _ g_le_one) ∅ hdisj
  rw [Cache.empty_extend] at h
  exact h

theorem regroup (G : Rec → (Message paperParams × A.State) × Cache paperParams → ℝ≥0∞) :
    ∑ ξ, w * E (run paperParams (A.choose (pkOf ξ)) ∅) (G ξ) =
      ∑ pk, E (run paperParams (A.choose pk) ∅) (fun p => ∑ ξ ∈ fiberA pk, w * G ξ p) := by
  symm
  calc ∑ pk, E (run paperParams (A.choose pk) ∅) (fun p => ∑ ξ ∈ fiberA pk, w * G ξ p)
      = ∑ pk, ∑ ξ ∈ fiberA pk, w * E (run paperParams (A.choose (pkOf ξ)) ∅) (G ξ) := by
        refine Finset.sum_congr rfl fun pk _ => ?_
        rw [← sum_mul_E_asm]
        refine Finset.sum_congr rfl fun ξ hξ => ?_
        rw [mem_fiberA_asm hξ]
    _ = ∑ ξ, w * E (run paperParams (A.choose (pkOf ξ)) ∅) (G ξ) := by
        unfold fiberA
...
theorem main_bound {B : ℕ} (hB : CostAtMost paperParams (GScheme.experiment forestScheme A) B) (hB' : B ≤ 2 ^ 127) :
  ...
  calc ∑ ξ : Rec, w * E (run paperParams (rest A (pkOf ξ, graph.evalRec ξ)) (kc ξ)) g
    ...
    _ = ∑ pk, E (run paperParams (A.choose pk) ∅) (fun p => ∑ ξ ∈ fiberA pk, w *
          (if Cache.Hits p.2 (kc ξ) then 1 else
            E (run paperParams (rest₂ A (pkOf ξ) (graph.evalRec ξ) p.1) (Cache.extend p.2 (kc ξ))) g)) :=
        regroup A (fun ξ p => if Cache.Hits p.2 (kc ξ) then 1 else
          E (run paperParams (rest₂ A (pkOf ξ) (graph.evalRec ξ) p.1) (Cache.extend p.2 (kc ξ))) g)
    _ = ∑ pk, E (run paperParams (A.choose pk) ∅) (fun p => FA A pk p.1 p.2) := by
        refine Finset.sum_congr rfl fun pk _ => ?_
